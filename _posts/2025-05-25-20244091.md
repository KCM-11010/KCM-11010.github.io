...
For later use, we define the *orbit* of $x\in\mathcal{X}$, denoted as $G\cdotp x \triangleq \lbrace \rho(g)x:g\in G\rbrace$. The collection of all orbits are denoted as $\mathcal{X}/G$. We also define the *stabilizer* of $x$, denoted as $G_x \triangleq \lbrace g\in G: \rho(g)x = x \rbrace$.
...
## Limitation of CN in continuity: Weighted FA

The answer for the guarantee in continuous equivariant function via CN is given by <d-cite key="dym2024equivariant"></d-cite>. Surprisingly, a continuous CN does not exist for rotation and symmetric groups even if the model $\phi$ itself is continuous. How does this counter-intuitive argument hold?

Let us see how. <d-footnote> The authors assumes compactness of group $G$ and the continuity of group action $G$ on domain (and codomain). Readers may refer to Appendix A of <d-cite key="dym2024equivariant"></d-cite> for more details. </d-footnote> Let $F(V, W)$ denote the space of functions from $V$ to $W, and let $C(V, W)$ denote the subset of these functions which are also continuous. We say a linear operator $\mathcal{E}: F(V, W)\to F(V, W)$ is a

$$
\begin{align*}
    \text{Bounded operator}: \forall \text{ compact } K, \exists C_K > 0 \;\;\text{s.t.}\;\; \max_{v\in K} \|\mathcal{E}(f)(v)\| \leq C_K \max_{v\in K} \|f(v)\|.\\
    \text{Equivariant projection operator}: \forall f\in F(V,W), \mathcal{E}(f) \text{ is equivariant. If } f \text{ is already equivariant, then } \mathcal{E}(f) = f.\\
    \text{Continuity-Preserving}: f\in C(V, W) \Rightarrow \mathcal{E}(f)\in C(V, W).
\end{align*}
$$
...
* Using weighted frame averaging, both the continuity and the in/equivariance of the model are satisfied.