...
where $G_x$ is the stabilizer of $x$, $G_x \triangleq \lbrace g\in G: \rho(g)x = x \rbrace$. <d-footnote> so, the problematic inputs for standard CN are those with non-trivial stabilizers </d-footnote> Moreover, the model $\phi$ in its canonicalized form satisfies the relaxed equivariance condition if CN function $h$ satisfies the relaxed equivariance condition. Whether how much the relaxed equivariance aligns with the standard equivariance in other literature should be discussed.
...
In this blog post, we have seen a series of methods that are able to inject in/equivariance of a given symmetry group to existing architectures. The three take-home messages to keep in mind are the following:
